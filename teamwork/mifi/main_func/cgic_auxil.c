...
int parse_input(char *pinput, struct data_from_web *pstru)
{
    ...
    else
    {
        ...
        if ( NULL != pjson_val )
        {
            strncpy(pstru->remark, pjson_val->valuestring, MAX_REMARK);
            pstru->remark[MAX_REMARK - 1] = '\0';
        }
        ...
        if ( NULL != pjson_val )
        {
            strncpy(pstru->module, pjson_val->valuestring, MAX_MODULE);
            pstru->module[MAX_MODULE - 1] = '\0';
        }
        ...
        if ( NULL != pjson_val )
        {
            strncpy(pstru->ip, pjson_val->valuestring, MAX_IP);
            pstru->ip[MAX_IP - 1] = '\0';
        }
...
/******************************************************************************
  ...
  Others:
*******************************************************************************/
void output_test(struct data_from_web *pinfo)
{
    /* create JSON object */
